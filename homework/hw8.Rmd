...
**Yes the data is tidy because the observations are in their own columns and not mixed up**
...
avg_cfu <- sydneybeaches_long %>% 
  separate(date, into=c("day", "month", "year"), sep="/") %>% 
  group_by(site, year) %>% 
  summarize(mean_cfu=mean(enterococci_cfu_100ml), .groups= 'keep')
```
...
avg_cfu %>% 
  group_by(site) %>% 
  summarise(max_cfu=max(mean_cfu,  na.rm = T)) %>% 
  arrange(desc(max_cfu))
```
