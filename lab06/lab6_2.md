...
## ℹ Specify the column types or set `show_col_types = FALSE` to quiet this message.
```
Told are how to interpret data and filter out -99 and (-) and replace with na
...
```r
superhero_info %>% 
  tabyl(publisher) #The publisher that has the highest number of superheros is Marvel Comics
```
...
```r
good_guys %>% 
  filter(race=="Vampire") #The vampires are Angel and Blade
```
...
## 2 Blood… Female blue      Human Brown         218 Marvel C… <NA>       bad      
## 3 Docto… Male   brown     Human Brown         201 Marvel C… <NA>       bad      
## 4 Kingp… Male   blue      Human No Hair       201 Marvel C… <NA>       bad      
## 5 Lizard Male   red       Human No Hair       203 Marvel C… <NA>       bad      
## 6 Scorp… Male   brown     Human Brown         211 Marvel C… <NA>       bad      
## # ℹ 1 more variable: weight <dbl>
```
...
```r
good_guys %>% 
  filter(hair_color == "Green") #There are more good guys with green hair with a total of 7. Bad guys only have 1 person with green hair
```
...
height_weight_ratio %>% 
  ...
  arrange(desc(ratio)) #The superhero with the highest height to weight ratio is Groot
```
...
superhero_powers %>% 
  ...
  filter(stamina==T) #There is a total of 40 superheros with agility, stealth, super_strength, and stamina
```
...
##   <chr>       <lgl>   <lgl>               <lgl>             
## 1 Black Widow TRUE    FALSE               FALSE             
