...
### Every variable in its own column and every observation is in its own row and each cell had one value = tidydata
...
```{r}
names(fish) #Six variable of interest 
```
...
```{r}
fish_subset <-select(fish, "fish_id", "length") #Useful for datasets with high variables 
```
...
```{r}
select(fish, matches("a.+er")) #Handy bit of code for project; look at variables of interest with "a" and neds with "er"
```
...
```{r}
mammals_new <- rename(mammals, genus="Genus", wean_mass="wean mass", max_life="max. life", litter_size="litter size", litters_year="litters/year")
```
