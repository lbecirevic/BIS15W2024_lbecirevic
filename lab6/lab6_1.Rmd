...
```{r}
fish %>% 
  mutate(length_mm = length*10) %>% #conversion of length to mm by multiplying by 10
  select(fish_id, length, length_mm) #variable doesnt go away
```
...
```{r}
mammals %>%
  mutate_all(tolower) #makes all data lowercase
```

Using the across function we can specify individual columns.
```{r}
mammals %>% 
  mutate(across(c("order", "family"), tolower)) #makes specific data lowercase
```
